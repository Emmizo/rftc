...
<div class="panel-heading"><center><strong>Leave Car</strong></center></div>
	<?php
	...
	or die(mysqli_error($conn));
		...
		$query4=mysqli_query($conn,"UPDATE setting_table SET time_leave='$time_leave',time_lease='$time_lease',status='$status' where licence_drive_id='$licence_drive_id' AND ID='$ID' AND road_id='$road_id' AND plack='$plack'")or die(mysqli_error($conn));

		$query=mysqli_query($conn,"INSERT INTO `archive` ( `ID`, `licence_drive_id`, `plack`, `road_id`, `time_leave`, `time_lease`,`date_done`, `status`) VALUES ('$ID', '$licence_drive_id', '$plack', '$road_id', '$time_leave', '$time_lease','$date_done', '$status');")or die(mysqli_error($conn));
header("location:./viewreport.php");
	}
	else{
echo '<center><strong style="color:red">Check car is not match with driver info,</b></strong></center>';
}}
	?>
    			...
    			<div class="row"  >
							...
							<div class=" input-field col-lg-12 col-md-12 col-sm-12" >
								...
								<select name="ID" class="form-control name">
                    				...
                    				inner join setting_table ON drivers.ID=setting_table.ID WHERE setting_table.status='Free'";
							...
							<div class=" input-field col-lg-12 col-md-12 col-sm-12">
								...
								<input type="hidden" name="time_leave" value="<?php echo "Taken at,".date('H:i:s'); ?>">
								<input type="hidden" name="time_lease" value="not available">
								<input type="hidden" name="date_done" value="<?php echo date('d/m/Y')?>">
